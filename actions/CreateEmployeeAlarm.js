"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const _1 = require("../infrastructure/dependency-injection/");
const ActionBase_1 = require("./ActionBase");
const Exceptions = require("../infrastructure/exceptions/");
const Entities = require("../entities/");
class Action extends ActionBase_1.ActionBase {
    constructor() {
        super();
        this.messageService = _1.kernel.get(_1.Types.IMessageService);
        this.employeeRepo = _1.kernel.get(_1.Types.IEmployeeRepository);
        this.employeeHistoryService = _1.kernel.get(_1.Types.IEmployeeHistoryService);
        this.userRepo = _1.kernel.get(_1.Types.IUserRepository);
    }
    ;
    execute(context) {
        return __awaiter(this, void 0, void 0, function* () {
            let message = {
                id: null,
                content: context.params.content,
                createdAt: (new Date()).toISOString(),
                deleted: false,
                executionType: Entities.MessageExecutionType.SCHEDULED,
                origin: context.params.origin,
                originName: null,
                scheduledAt: context.params.scheduledAt,
                seen: false,
                sent: false,
                sentAt: null,
                target: Entities.MessageType.ALL,
                targetName: Entities.MessageType.ALL,
                type: Entities.MessageType.ALL,
                status: Entities.MessageStatus.OPEN,
                assignee: null,
                assigneeName: null,
                notificationType: Entities.NotificationType.INFO,
            };
            yield this.messageService.create(message);
            let employee = yield this.employeeRepo.findEmployee(context.params.employeeId);
            if (!employee.dossier.alarms) {
                employee.dossier.alarms = [];
            }
            employee.dossier.alarms.push({
                message: message.content,
                scheduledAt: message.scheduledAt,
            });
            yield this.employeeHistoryService.createRecord(employee, "Create alarm for employee", message, context.params.user.name + " " + context.params.user.lastName);
            return yield this.employeeRepo.updateEmployee(employee);
        });
    }
    onActionExecuting(context) {
        return __awaiter(this, void 0, void 0, function* () {
            let user = yield this.userRepo.findUser(context.params.userId);
            if (!user) {
                throw new Exceptions.EntityNotFoundException("User", context.params.userId);
            }
            context.params.user = user;
            return context;
        });
    }
    onActionExecuted(result) {
        return __awaiter(this, void 0, void 0, function* () {
            return result;
        });
    }
    getConstraints() {
        return {
            employeeId: "string|required",
            origin: "string|required",
            content: "string|required",
            scheduledAt: "string|required",
        };
    }
    getSanitizationPattern() {
        return {};
    }
}
exports.Action = Action;
//# sourceMappingURL=CreateEmployeeAlarm.js.map